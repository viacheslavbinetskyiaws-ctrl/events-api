...
- `wal_level=logical` has to be enabled in **three** places, or CDC works locally and silently doesn't in-cluster: `docker-compose.yml`'s postgres service (`command: ["postgres", "-c", "wal_level=logical", ...]`, needs a container restart), `k8s/base/postgres-deployment.yaml`, and the separately `sed`-copied Postgres manifests in the `events-api-helm` namespace (Milestone 6 Part B).
