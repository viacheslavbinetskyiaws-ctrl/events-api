...
- Originally planned as "point `stg_events`/a mart at the BigQuery target via `--target bigquery`" — turned out impossible as written, confirmed while actually building this milestone: `source('app', 'events')` resolves target-relative, so under `--target bigquery` it becomes `<bq-project>.public.events`, which doesn't exist — the real data physically lives in Postgres only, and this milestone deliberately doesn't migrate or replicate the primary pipeline (see below). A per-target source override would only fix the naming, not the absent data. Actual approach: a standalone model under a new `models/bigquery/` path (own `dbt_project.yml` config block — the `marts/` directory's `+grants`/`+post-hook` are Postgres-only DDL and would break/misapply under the BigQuery adapter), generating its own synthetic data in-model (`UNNEST(GENERATE_ARRAY(...))`, not a `dbt seed` CSV — a handful of seeded rows wouldn't spread across enough partitions to make pruning actually *visible*, just configured) spanning enough distinct dates to demonstrate `partition_by`/`cluster_by` for real. Multi-tenant isolation (authorized views or row-level access policies — BigQuery has no session-scoped RLS equivalent, genuinely different from `PLAN.md` Milestone 7's Postgres RLS) is its own separate follow-up step with its own verification, not bundled into the first successful `dbt run --target bigquery`.
